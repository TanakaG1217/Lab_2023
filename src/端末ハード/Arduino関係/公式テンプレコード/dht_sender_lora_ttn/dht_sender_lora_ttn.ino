...
void loop() {
  ...
  if (isnan(event.relative_humidity)) {
    ...
    float fractional_temp = (DHT11_temp -(int)DHT11_temp)*100;
    float fractional_hum = (DHT11_hum -(int)DHT11_hum)*100;
    //confirm status,Fport,payload length,payload(HEX)
    snprintf(sensor_data_buff,128,"AT+SENDB=%d,%d,%d,%02X%02X%02X%02X",0,2,4,(int)DHT11_temp,(int)fractional_temp,(int)DHT11_hum,(int)fractional_hum);
