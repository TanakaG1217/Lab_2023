...
def sendGpsData():
    if gps.clean_sentences > 20:  # GPSsentenceが２０文字たまったら出力する
        ...
        while try_count < max_try_count:
            ...
            # True or False
            have_TxDone = "txDone" in readed_serial_str
            have_RxDone = "rxDone" in readed_serial_str
            have_rxTimeout = "rxTimeout" in readed_serial_str

            if have_TxDone and have_RxDone:
                print("送信成功")
                logger.info("UL sended")
                break
            elif have_TxDone and have_rxTimeout:
                print("送信タイムアウト")
                time.sleep(4.0)
            else:
                print("送信エラー")
